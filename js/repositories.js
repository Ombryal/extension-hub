...
const RepositoryView = {
  ...
  createCard(repository) {
    ...
    if (Array.isArray(repository.sourceStatus)) {
      const online = repository.sourceStatus.filter(
        source => source.status === "online"
      ).length;

      const health = App.createElement("span", "repository-card-count");
      health.textContent = `${online}/${sources} sources online`;
